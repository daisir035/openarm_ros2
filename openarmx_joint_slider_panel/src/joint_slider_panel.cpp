...
JointSliderPanel::JointSliderPanel(QWidget * parent)
...
void JointSliderPanel::setupUi()
{
  ...
  auto * title = new QLabel("<h3>OpenArm 关节滑块面板</h3>");
  ...
  connect(
    end_effector_mode_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged),
  ...
  connect(
    joint_step_slider_, &QSlider::valueChanged, this,
    &JointSliderPanel::onJointStepSliderChanged);
  connect(
    gripper_step_slider_, &QSlider::valueChanged, this,
    &JointSliderPanel::onGripperStepSliderChanged);

  updateEndEffectorModeUi();
}

JointSliderPanel::SliderBinding JointSliderPanel::createJointSliderRow(
  const QString & title,
  ...
  double init_value)
{
  ...
  connect(
    slider, &QSlider::valueChanged, this, [this, binding](int) {
...
void JointSliderPanel::updateSliderLabel(const SliderBinding & binding)
{
  ...
  if (binding.is_o6) {
    ...
    const double rad = static_cast<double>(binding.slider->value()) /
      static_cast<double>(kArmSliderScale);
...
bool JointSliderPanel::applyUrdfJointLimits(const std::string & urdf_xml)
{
  ...
  auto apply_arm_joint_limit =
    [this, &model](SliderBinding & binding, const std::string & joint_name) {
  ...
  auto apply_gripper_joint_limit =
    [this, &model](SliderBinding & binding, const std::string & joint_name) {
      auto joint = model.getJoint(joint_name);
      if (!joint || !joint->limits) {
        return false;
...
void JointSliderPanel::onInitialize()
{
  ...
  left_arm_pub_ = node_->create_publisher<trajectory_msgs::msg::JointTrajectory>(
    "left_joint_trajectory_controller/joint_trajectory", 10);
  right_arm_pub_ = node_->create_publisher<trajectory_msgs::msg::JointTrajectory>(
...
void JointSliderPanel::jointStateCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  if (hasAllTargetJointStates()) {
...

    {
      ...
      } else if (!slider_interacting_ &&
        targetStatesApproxEqual(desired_target_, command_target_))
      {
        command_target_ = current_state;
        desired_target_ = current_state;
        should_track_live_state = true;
...
bool JointSliderPanel::applyJointStateToSliders()
{
  ...
  for (size_t i = 0; i < left_arm_sliders_.size(); ++i) {
    ...
    left_arm_sliders_[i].slider->setValue(
      std::clamp(
  ...
  for (size_t i = 0; i < right_arm_sliders_.size(); ++i) {
    ...
    const int raw = static_cast<int>(std::lround(rad * kArmSliderScale));
    right_arm_sliders_[i].slider->setValue(
      std::clamp(
  ...
  if (shouldUseGripperMode()) {
    const int left_mm =
      static_cast<int>(std::lround(
        latest_joint_state_map_.at(left_gripper_joint_name_) *
        1000.0));
    const int right_mm =
      static_cast<int>(std::lround(
        latest_joint_state_map_.at(right_gripper_joint_name_) *
        1000.0));
    ...
    right_gripper_slider_.slider->setValue(
      std::clamp(
        right_mm, right_gripper_slider_.slider->minimum(),
        right_gripper_slider_.slider->maximum()));
  }
...
void JointSliderPanel::onHandsUpClicked()
{
  ...
  right_gripper_slider_.slider->setValue(
    std::clamp(
      0, right_gripper_slider_.slider->minimum(),
      right_gripper_slider_.slider->maximum()));
...
bool JointSliderPanel::setupPreviewModel(const std::string & urdf_xml)
{
  ...
  if (!kdl_parser::treeFromString(urdf_xml, preview_kdl_tree_)) {
    ...
    if (node_) {
      RCLCPP_ERROR(
        node_->get_logger(), "setupPreviewModel failed: kdl_parser::treeFromString failed");
    }
    return false;
  }
  ...
  if (node_) {
    RCLCPP_INFO(
      node_->get_logger(), "setupPreviewModel ok: root=%s, segments=%u",
...
void JointSliderPanel::publishPreviewTransforms()
{
  ...
  for (size_t i = 0; i < right_arm_joint_names_.size() && i < preview_target.right_arm.size();
    ++i)
  {
...
bool JointSliderPanel::stepTowardsTarget(
  TargetState & current, const TargetState & target,
...
bool JointSliderPanel::targetStatesApproxEqual(
  const TargetState & lhs,
  const TargetState & rhs) const
{
...
trajectory_msgs::msg::JointTrajectory JointSliderPanel::makeTrajectory(
...
bool JointSliderPanel::sendCommands(const TargetState & target)
{
  if (!left_arm_pub_ || !right_arm_pub_) {
    return false;
  }

  left_arm_pub_->publish(makeTrajectory(left_arm_joint_names_, target.left_arm));
  right_arm_pub_->publish(makeTrajectory(right_arm_joint_names_, target.right_arm));

  if (shouldUseGripperMode() && left_gripper_pub_ && right_gripper_pub_) {
    left_gripper_pub_->publish(
      makeTrajectory({left_gripper_joint_name_}, {target.left_gripper}));
    right_gripper_pub_->publish(
      makeTrajectory({right_gripper_joint_name_}, {target.right_gripper}));
  }
...
void JointSliderPanel::updateStatusText()
{
  ...
  const QString mode_str = shouldUseGripperMode() ? "夹爪模式" : "手部模式(7轴+O6)";
  setStatus(
    QString("控制: %1 轨迹控制(分段执行) | %2 | %3 | %4 | 预览: 半透明实时(按滑块目标, 不受步长影响)")
    .arg(mode_str)
