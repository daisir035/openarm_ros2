...
def save_yaml(
        file_path: str, joint_names: List[str],
        points: List[List[float]], dt: float) -> None:
    data = {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Continuously record /joint_states to YAML')
    parser.add_argument(
        '--topic', default='',
        help='Joint states topic name (overrides --arm-prefix)')
    parser.add_argument(
        '--arm-prefix', default='',
        help='ROS namespace prefix used to find the joint_states topic')
    parser.add_argument('--outfile', default='', help='Output YAML file path')
    parser.add_argument(
        '--rate', type=float, default=10.0,
        help='Recording rate in Hz when running')
